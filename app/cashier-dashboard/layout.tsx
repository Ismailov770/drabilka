...
export default function CashierLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    { label: "Dashboard", href: "/cashier-dashboard", icon: <LayoutDashboard className="w-5 h-5" /> },
    { label: "Sales Management", href: "/cashier-dashboard/sales", icon: <ShoppingCart className="w-5 h-5" /> },
    { label: "Expenses (Rashod)", href: "/cashier-dashboard/expenses", icon: <Receipt className="w-5 h-5" /> },
    { label: "Payroll", href: "/cashier-dashboard/payroll", icon: <Users className="w-5 h-5" /> },
    { label: "Debts / Qarzlar", href: "/cashier-dashboard/debts", icon: <CreditCard className="w-5 h-5" /> },
  ]
    ...
    <div className="min-h-screen bg-[#F8FAFC]">
      ...
      <header className="md:hidden flex items-center justify-between px-4 py-3 border-b border-slate-200 bg-white sticky top-0 z-30">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-slate-900">SimentMaker · Cashier</span>
        </div>
        <button
          type="button"
